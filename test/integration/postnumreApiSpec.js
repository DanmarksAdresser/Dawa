...
describe("PostnumreApi", function() {
  it("It is possible to get a single postnummer", function(done) {
    request.get({url: "http://localhost:3000/api/pg/postnumre/8600", json: true}, function(error, response, result) {
      ...
      expect(result.kommuner).toEqual([{ href : 'http://dawa.aws.dk/api/pg/kommuner/740', kode : 740, navn : 'Silkeborg' }]);
      done();
    });
  });
  ...
  it('It is possible to search for postnumre', function(done) {
    request.get({url: "http://localhost:3000/api/pg/postnumre?q=silkeborg", json: true}, function(error, response, result) {
      ...
      expect(postnummer.kommuner).toEqual([{ href : 'http://dawa.aws.dk/api/pg/kommuner/740', kode : 740, navn : 'Silkeborg' }]);
      done();
    });
  });
