...
describe('Autocomplete af adresser', function() {
  it('Should be possible to autocomplete an address', function(done) {
    request.get({url: 'http://localhost:3000/api/pg/adresser/autocomplete?q=Solv', json: true}, function(error, response, result) {
  ...
  it('Should accept filtering parameter', function(done) {
    request.get({url: 'http://localhost:3000/api/pg/adresser/autocomplete?q=solv&vejkode=7750&kommunekode=253', json: true}, function(error, response, result) {
      expect(result.length).toBe(6);
      done();
    });
  });
