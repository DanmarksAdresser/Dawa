...
CREATE VIEW dar1_vejstykkerpostnumremat_view AS
  ...
  from dar1_NavngivenVejKommunedel_current nvk
    ...
    dar1_postnummer_current p
WHERE EXISTS(SELECT * FROM dar1_husnummer_current  h WHERE h.navngivenvej_id = nv.id AND h.darkommune_id = k.id AND h.postnummer_id = p.id );
